...
export default function PregnancyNew() {
  ...
  const handleSearch = async () => {
    if (nidSearch.length === 10) {
      ...
      if (res.data?.patient) {
        ...
        toast({ title: "غير موجودة", description: "لم يتم العثور على المريضة", variant: "destructive" });
      }
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {/* Patient Lookup */}
      <Card>
        <CardHeader><CardTitle>{t("pregnancy.patientLookup")}</CardTitle></CardHeader>
        <CardContent>
          ...
          {patientFound && (
            <div className="mt-4 p-4 bg-muted rounded-md grid grid-cols-2 gap-2 text-sm">
              <div><span className="text-muted-foreground">الاسم: </span><span className="font-medium">{patientFound.nameAr}</span></div>
              <div><span className="text-muted-foreground">المركز: </span><span className="font-medium">{patientFound.healthCenterNameAr}</span></div>
              <div><span className="text-muted-foreground">العمر: </span><span className="font-medium">{patientFound.age != null ? `${patientFound.age} سنة` : "—"}</span></div>
              <div><span className="text-muted-foreground">الجوال: </span><span className="font-medium" dir="ltr">{patientFound.phone}</span></div>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className={`space-y-6 ${!hasPatient ? "opacity-50 pointer-events-none" : ""}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="gestationalAge" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input type="number" min={0} max={45} {...field} value={field.value || ""} placeholder="0" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {Object.values(PregnancyInputRiskLevel).map(l => (
                          <SelectItem key={l} value={l}>{t(`risk.${l}` as any)}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <FormMessage />
                ...
                <FormField control={form.control} name="doctorName" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="اسم الطبيب" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {RISK_FACTORS_G1.map((factor) => (
                  <FormField key={factor} control={form.control} name="riskFactors" render={({ field }) => (
                    <FormItem className="flex flex-row items-center gap-3 rounded-md border p-3 space-y-0">
                      ...
                      <FormLabel className="font-normal text-sm cursor-pointer">{factor}</FormLabel>
                    </FormItem>
                  )} />
                ))}
              </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {RISK_FACTORS_G2.map((factor) => (
                  <FormField key={factor} control={form.control} name="pregnancyRiskFactors" render={({ field }) => (
                    <FormItem className="flex flex-row items-center gap-3 rounded-md border p-3 space-y-0">
                      <FormControl>
                        <Checkbox
                          checked={field.value?.includes(factor)}
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {RISK_FACTORS_G3.map((factor) => (
                  <FormField key={factor} control={form.control} name="medicalConditions" render={({ field }) => (
                    <FormItem className="flex flex-row items-center gap-3 rounded-md border p-3 space-y-0">
                      <FormControl>
                        <Checkbox
                          checked={field.value?.includes(factor)}
            ...
            <CardContent className="space-y-4">
              <FormField control={form.control} name="medications" render={({ field }) => (
                <FormItem>
                  ...
                  <p className="text-xs text-muted-foreground">{t("pregnancy.medicationsDesc")}</p>
                  <FormControl><Textarea {...field} placeholder="اذكر الدواء إن وُجد..." rows={2} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField control={form.control} name="isVteHighRisk" render={({ field }) => (
                  <FormItem className="flex flex-row items-center gap-3 rounded-md border p-4 space-y-0">
                    <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                    <FormLabel className="cursor-pointer">{t("pregnancy.isVteHighRisk")}</FormLabel>
                  </FormItem>
                )} />
                <FormField control={form.control} name="enoxaparinPrescribed" render={({ field }) => (
                  <FormItem className="flex flex-row items-center gap-3 rounded-md border p-4 space-y-0">
                    <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                    <FormLabel className="cursor-pointer">{t("pregnancy.enoxaparinPrescribed")}</FormLabel>
                  </FormItem>
                )} />
              </div>

              <FormField control={form.control} name="referralRecommendation" render={({ field }) => (
                <FormItem>
              ...
              <FormField control={form.control} name="referralExplained" render={({ field }) => (
                <FormItem>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField control={form.control} name="referredHospitalId" render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      <FormControl><SelectTrigger><SelectValue placeholder="اختر المستشفى" /></SelectTrigger></FormControl>
                      <SelectContent>
            ...
            <CardContent className="space-y-4">
              <FormField control={form.control} name="notes" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea {...field} placeholder="ملاحظات عامة..." rows={2} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="followUpNotes" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea {...field} placeholder="استجابات تواصل المراجعة..." rows={2} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </CardContent>

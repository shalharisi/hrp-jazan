...
export default function PatientDetail() {
  ...
  const { data: patient, isLoading: loadingPatient, refetch } = useGetPatient(patientId, {
          ...
          {editMode ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={patient.age != null ? `${patient.age} سنة` : "—"} disabled className="bg-muted" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.doctorPhone} onChange={e => setForm(f => ({ ...f, doctorPhone: e.target.value }))} dir="ltr" placeholder="05XXXXXXXX" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={selectedSectorId ? String(selectedSectorId) : (patient.sectorId ? String(patient.sectorId) : undefined)}
                  onValueChange={v => {
                ...
                >
                  ...
                  <SelectContent>
                    {sectors?.map(s => <SelectItem key={s.id} value={String(s.id)}>{s.nameAr}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5 md:col-span-2">
                ...
                >
                  ...
                  <SelectContent>
                    {healthCenters?.map(hc => <SelectItem key={hc.id} value={String(hc.id)}>{hc.nameAr}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <InfoRow icon={<User className="w-4 h-4 text-muted-foreground" />} label={t("patients.nationalId")} value={patient.nationalId} dir="ltr" />
              <InfoRow icon={<User className="w-4 h-4 text-muted-foreground" />} label={t("patients.name")} value={patient.nameAr} />
              <InfoRow icon={<Calendar className="w-4 h-4 text-muted-foreground" />} label={t("patients.dateOfBirth")} value={dobFormatted ?? "—"} />
              <InfoRow icon={<Calendar className="w-4 h-4 text-muted-foreground" />} label={t("patients.age")} value={patient.age != null ? `${patient.age} سنة` : "—"} />
              <InfoRow icon={<Phone className="w-4 h-4 text-muted-foreground" />} label={t("patients.phone")} value={patient.phone} dir="ltr" />
              <InfoRow icon={<Stethoscope className="w-4 h-4 text-muted-foreground" />} label={t("patients.doctorPhone")} value={patient.doctorPhone ?? "—"} dir="ltr" />
              <InfoRow icon={<MapPin className="w-4 h-4 text-muted-foreground" />} label={t("patients.address")} value={patient.address ?? "—"} />
              <InfoRow icon={<MapPin className="w-4 h-4 text-muted-foreground" />} label={t("patients.sector")} value={patient.sectorNameAr ?? "—"} />
              <InfoRow icon={<MapPin className="w-4 h-4 text-muted-foreground" />} label={t("patients.healthCenter")} value={patient.healthCenterNameAr ?? "—"} />
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <Table>
            ...
            <TableBody>
              {loadingPregnancies ? (
                ...
                <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">{t("general.noData")}</TableCell></TableRow>
              ) : (
                pregnancies?.items.map(p => (
                  <TableRow key={p.id}>
                    <TableCell>{new Date(p.visitDate).toLocaleDateString("ar-SA")}</TableCell>
                    <TableCell>{p.gestationalAge != null ? `${p.gestationalAge} أسبوع` : "—"}</TableCell>

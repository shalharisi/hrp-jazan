...
import { PregnancyRiskLevel, PregnancyCompliance, PregnancyReferralRecommendation } from "@workspace/api-client-react";
...
export function ComplianceBadge({ status }: { status?: string }) {
  ...
  const config: Record<string, { cls: string; icon: string }> = {
    [PregnancyCompliance.compliant]: { cls: "bg-green-500/10 text-green-700 border-green-500/20", icon: "✓" },
    [PregnancyCompliance.non_compliant]: { cls: "bg-red-500/10 text-red-700 border-red-500/20", icon: "✗" },
    [PregnancyCompliance.pending]: { cls: "bg-gray-500/10 text-gray-700 border-gray-500/20", icon: "⏳" },
  };
...
export function ReferralBadge({ recommendation }: { recommendation?: string }) {
  ...
  return (
    <Badge variant="secondary">
      {t(`referral.${recommendation}` as any) || recommendation}
    </Badge>
  );
}

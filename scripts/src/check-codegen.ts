...
const currentSpecHash = createHash("sha256").update(readFileSync(specPath)).digest("hex");

if (!existsSync(hashPath)) {
  console.error("check:codegen — .codegen-hash is missing. Generated files may be out of sync.");
...
if (currentSpecHash !== storedSpecHash) {
  console.error("check:codegen — openapi.yaml has changed but codegen has not been run.");
  console.error("  The generated hooks and Zod schemas are out of sync with the spec.");
  console.error("  Run: pnpm --filter @workspace/api-spec run codegen");
  process.exit(1);
}
...
if (missingDirs.length > 0) {
  console.error("check:codegen — generated output directories are missing entirely:");
...
if (currentOutputHash !== storedOutputHash) {
  console.error("check:codegen — generated files have been manually edited or are out of sync.");
  console.error(
